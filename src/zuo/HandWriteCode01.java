...
import static zuo.HandWriteCode01.BucketSort.maxGap;
import static zuo.sort.QuickSort.quickSort;

public class HandWriteCode01 {
...
// 6.堆排序!
    ...
    static class BucketSort {
        /**
         * 假设数据范围 0~n ，有上亿个元素，请排序。
         * 实现计数排序，用数组计数每个桶中元素出现的频次。
         *
         * @param arr
         */
        public static void bucketSort(int[] arr) throws Exception {
            ...
            for (int i = 0; i < bucket.length; i++) {
                while (bucket[i]-- > 0) {// 将第i号桶里的数全部倒入数组
                    arr[index++] = i;
                }
            }
        }
        ...
        /**
         * 桶思想拓展题：求一个无序数组，在其有序之后相邻元素最大差值，要求时间复杂度O(n).
         * 当数组元素值范围很大时比如是long类型时，无法使用桶排序来求解。因为要为每个值创建一个桶
         * 内存不够用！但是这道题需要借助桶思想来解决。
         * 求解：
         * 1.遍历数组确定范围的min、max，以及元素个数n
         * 2.建立n+1个桶，均分范围min~max。每个桶则负责一段值范围
         * 3.将数组的n个元素，按其值放到对应的桶中。其中min桶必有min值，max桶必有max值，因为有n+1桶，那么中间至少有一个空桶。
         * <p>
         * 数组排序后相邻的元素，可能来自同一个桶，也可能来自不同的桶。比如空桶两侧的2个非空桶，排序后，左桶的最大值一定和右桶最小值就相邻。
         * 当相邻数来自一个桶时，其差值一定小于桶负责的范围。而当相邻元素来自空桶两侧的2个非空桶时，其差值(右桶min-左桶max)一定大于一个桶负责的范围。
         * 所以最大差值不可能来自一个桶，但也不一定来自空桶两侧的非空桶的相邻数！也有可能是2个相邻非空桶中的相邻元素！！
         * <p>
         * 求解：只需要记录每个桶中的最大最小值，依次求解相邻非空桶或中间隔着空桶(可能多个)的“相邻非空桶”，它们的差值：即右桶min-左桶min，其中最大的差值即为解。
         * <p>
         * 注：分析空桶只是为了说明相邻最大差值不可能来自一个桶中的元素，仅此而已！
         * 桶的边界可以是小数，这样才能保证将min~max范围均分。实际上，求解时根本不需要桶，只需要知道将哪个元素放入哪个桶，然后记录桶中元素最大最小值即可！
         */
        public static int maxGap(int[] arr) {
            ...
            int len = arr.length;
            // 2.使用3个数组记录桶的信息，共len+1个桶，编号：0~len
            boolean[] hasNum = new boolean[len + 1];
            int[] mins = new int[len + 1];
            int[] maxs = new int[len + 1];

            int bid = 0; // 桶编号
            // 遍历数组，将元素模拟放进对应桶，更新桶信息
            for (int i = 0; i < arr.length; i++) {
                bid = bucketId(arr[i], min, max, len);
                // 桶中是否已有最大最小值，若有则比较
         ...
         * @param len    桶编号0~len
         */
        public static int bucketId(int target, int min, int max, int len) {
            return (int)((target - min)*1.0 * len / (max - min)); // 这里最好使用浮点数计算然后强制转换。
            // 若使用int乘除，因为截断可能导致乘除顺序不同时，结果完全不一样。
        }
    ...
    public static void main(String[] args) {
        ...
        int[] arr3 = MyUtils.generatePosRandomArray(2000, 50);
        System.out.println("arr before:" + Arrays.toString(arr3));
        try {
            BucketSort.bucketSort(arr3);
        } catch (Exception e) {
            e.printStackTrace();
        }
        System.out.println("arr after:" + Arrays.toString(arr3));

//        int[] arr4 = MyUtils.generatePosRandomArray(5, 20);
        int[] arr4 = {3, 5, 11, 14, 17};
        System.out.println(Arrays.toString(arr4));
        System.out.println("max gap:");
        System.out.println(maxGap(arr4));
        System.out.println("after algorithm.sort:");
        quickSort(arr4); // 为了便于验证，实际不用排序
        System.out.println(Arrays.toString(arr4));
