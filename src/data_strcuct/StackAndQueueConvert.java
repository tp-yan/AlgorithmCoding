...
public class StackAndQueueConvert {
    /**
     * 使用2个队列实现栈：只能运用已有的队列结构，而不能修改原队列。
     * 由于队列只能先进先出，栈只能先进后出，假设queue存放数据，当进行栈peek和pop操作时，可以将queue的前n-1个元素出队，
     * 再入队到help队列中，然后得到queue最后一个元素n，若是peek操作，则返回n后，再将n入队help，若是pop操作则不同，最后
     * 交换queue，help的引用，因为help存放了数据，而queue为空作为辅助队列
     */
